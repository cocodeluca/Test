...
import { CheckCircle2, Database, Download, FileText, Globe2, Landmark, LifeBuoy, Lock, Monitor, Palette, PlayCircle, RefreshCw, Settings2, ShieldCheck, Upload, UserCircle2 } from 'lucide-react';
...
import { areFxRatesStale } from '../../../common/utils/fxRates';
...
export const SettingsPage: React.FC<SettingsPageProps> = ({ currentUser, onExportBackup, onImportBackup, onSyncBackupToServer, onRestoreBackupFromServer, onReplayDemoTutorial, onResetDemoData }) => {
  const { settings, fxSyncStatus, refreshFxRates, updateProfile, updateSettings, t, formatPreviewCurrency, formatPreviewDate, formatPreviewNumber } = useSettings();
  ...
  useEffect(() => {
    let active = true;
    void isRemoteAccountBackupEnabled().then((enabled) => {
      if (active) setRemoteBackupEnabled(enabled);
    }).catch(() => {
      if (active) setRemoteBackupEnabled(false);
    });
    return () => { active = false; };
  }, []);
  const fxSnapshot = settings.fxSnapshot;
  const fxLastUpdatedAt = fxSnapshot?.lastSuccessfulUpdateAt ?? settings.fxRatesFetchedAt ?? null;
  const fxSource = fxSnapshot?.provider ?? (settings.usdToEurRateSource === 'ECB' ? 'ECB' : t('common.manual'));
  const fxIsStale = areFxRatesStale(settings);
  const formatFxDateTime = (value: string) => {
    const parsedDate = new Date(value);
    if (Number.isNaN(parsedDate.getTime())) {
      return t('common.notSpecified');
    }
    return new Intl.DateTimeFormat(settings.language === 'es' ? 'es-ES' : settings.language === 'pt' ? 'pt-PT' : 'en-GB', {
      dateStyle: 'medium',
      timeStyle: 'short',
    }).format(parsedDate);
  };
  ...
  const renderFxSettings = () => (
    <section className={sectionCardClass}>
      <div className="flex items-start gap-4">
        <div className={`${appIconChipClass} p-3 text-sky-600 dark:text-sky-300`}><RefreshCw className={`h-5 w-5 ${fxSyncStatus.isRefreshing ? 'animate-spin' : ''}`} /></div>
        <div><h2 className={`text-xl font-semibold ${appTextStrongClass}`}>{t('settings.fxSection.title')}</h2><p className={`mt-1 text-sm ${appTextMutedClass}`}>{t('settings.fxSection.description')}</p></div>
      </div>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className={`${appPanelInsetClass} rounded-2xl p-4`}><p className={`text-xs font-medium uppercase tracking-wide ${appTextSoftClass}`}>{t('settings.fxSection.base')}</p><p className={`mt-2 text-sm font-semibold ${appTextStrongClass}`}>{fxSnapshot?.baseCurrency ?? settings.currency}</p></div>
        <div className={`${appPanelInsetClass} rounded-2xl p-4`}><p className={`text-xs font-medium uppercase tracking-wide ${appTextSoftClass}`}>{t('settings.fxSection.provider')}</p><p className={`mt-2 text-sm font-semibold ${appTextStrongClass}`}>{fxSource}</p></div>
        <div className={`${appPanelInsetClass} rounded-2xl p-4`}><p className={`text-xs font-medium uppercase tracking-wide ${appTextSoftClass}`}>{t('settings.fxSection.lastUpdated')}</p><p className={`mt-2 text-sm font-semibold ${appTextStrongClass}`}>{fxLastUpdatedAt ? formatFxDateTime(fxLastUpdatedAt) : t('common.notSpecified')}</p></div>
        <div className={`${appPanelInsetClass} rounded-2xl p-4`}><p className={`text-xs font-medium uppercase tracking-wide ${appTextSoftClass}`}>{t('settings.fxSection.status')}</p><p className={`mt-2 text-sm font-semibold ${fxIsStale ? 'text-amber-700 dark:text-amber-300' : appTextStrongClass}`}>{fxSyncStatus.isRefreshing ? t('settings.fxSection.refreshing') : fxIsStale ? t('settings.fxSection.stale') : t('settings.fxSection.current')}</p></div>
      </div>
      {fxSyncStatus.warning ? <p className="mt-4 rounded-2xl border border-amber-300/70 bg-amber-50/80 px-4 py-3 text-sm text-amber-800 dark:border-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200">{fxSyncStatus.warning}</p> : null}
      <button type="button" disabled={fxSyncStatus.isRefreshing} onClick={() => void refreshFxRates({ force: true })} className={`mt-4 inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-sm font-medium ${appButtonPrimaryClass} disabled:cursor-not-allowed disabled:opacity-70`}><RefreshCw className={`h-4 w-4 ${fxSyncStatus.isRefreshing ? 'animate-spin' : ''}`} />{fxSyncStatus.isRefreshing ? t('settings.fxSection.refreshing') : t('settings.fxSection.refresh')}</button>
    </section>
  );
