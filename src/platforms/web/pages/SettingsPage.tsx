...
export const SettingsPage: React.FC<SettingsPageProps> = ({ currentUser, onExportBackup, onImportBackup, onSyncBackupToServer, onRestoreBackupFromServer, onReplayDemoTutorial, onResetDemoData }) => {
  ...
  const renderAccounts = () => (
    <div className="space-y-4 sm:space-y-6">
      <section className={sectionCardClass}>
        <div className="flex items-start gap-4">
          <div className={`${appIconChipClass} p-3 text-cyan-600 dark:text-cyan-300`}><Settings2 className="h-5 w-5" /></div>
          <div>
            <h2 className={`text-xl font-semibold ${appTextStrongClass}`}>{t('settings.currencyDisplayTitle')}</h2>
            <p className={`mt-1 text-sm ${appTextMutedClass}`}>{t('settings.currencyDisplayDescription')}</p>
          </div>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div>
            <label className={labelClass}>{t('settings.fields.displayMode')}</label>
            <select className={appInputClass} value={settings.displayMode ?? 'single-reporting-currency'} onChange={(e) => updateSettings({ displayMode: e.target.value as 'single-reporting-currency' | 'mixed-logical-currencies' })}>
              <option value="single-reporting-currency">{t('settings.options.singleReportingCurrency')}</option>
              <option value="mixed-logical-currencies">{t('settings.options.mixedLogicalCurrencies')}</option>
            </select>
            <p className={helperClass}>{t('settings.helpers.displayMode')}</p>
          </div>
          <div>
            <label className={labelClass}>{t('settings.fields.reportingCurrency')}</label>
            <select className={appInputClass} value={settings.reportingCurrency ?? settings.currency} onChange={(e) => updateSettings({ reportingCurrency: e.target.value as typeof settings.currency, currency: e.target.value as typeof settings.currency })}>
              {currencyOptions.map((currency) => (
                <option key={currency.code} value={currency.code}>{getLocalizedCurrencyLabel(currency.code)}</option>
              ))}
            </select>
            <p className={helperClass}>{t('settings.helpers.reportingCurrency')}</p>
          </div>
          <div>
            <label className={labelClass}>{t('settings.fields.valueCurrency')}</label>
            <select className={appInputClass} value={settings.valueCurrency ?? settings.currency} onChange={(e) => updateSettings({ valueCurrency: e.target.value as typeof settings.currency })}>
              {currencyOptions.map((currency) => (
                <option key={currency.code} value={currency.code}>{getLocalizedCurrencyLabel(currency.code)}</option>
              ))}
            </select>
            <p className={helperClass}>{t('settings.helpers.valueCurrency')}</p>
          </div>
          <div>
            <label className={labelClass}>{t('settings.fields.operatingCurrency')}</label>
            <select className={appInputClass} value={settings.operatingCurrency ?? settings.currency} onChange={(e) => updateSettings({ operatingCurrency: e.target.value as typeof settings.currency })}>
              {currencyOptions.map((currency) => (
                <option key={currency.code} value={currency.code}>{getLocalizedCurrencyLabel(currency.code)}</option>
              ))}
            </select>
            <p className={helperClass}>{t('settings.helpers.operatingCurrency')}</p>
          </div>
        </div>
      </section>
